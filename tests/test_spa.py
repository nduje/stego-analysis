...
Run:
    python -m pytest tests/test_spa.py
...
from analysis.spa import sample_pairs, _trace_counts, estimate_rate, analyze_image
...
def _natural_channel(h=256, w=256, seed=0):
    ...
    base = (0.5 * xx + 0.5 * yy) * (200.0 / (h + w))
    base = base + rng.normal(0, 3, size=(h, w))
...
def test_full_lsb_high_estimate():
    ch = _natural_channel()
    assert estimate_rate(_lsb_randomize(ch)) > 0.8


def test_trace_counts_partition():
    ch = _natural_channel()
    u, v = sample_pairs(ch)
    X, Y, W, Z, P = _trace_counts(u, v)
    assert X + Y + Z == P        # X, Y, Z partition all pairs
    assert 0 <= W <= Y           # W is a subset of Y
...
if __name__ == "__main__":
    tests = [
        ...
        ("trace counts partition", test_trace_counts_partition),
