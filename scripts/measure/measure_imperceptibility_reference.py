"""Imperceptibility for the reference methods (LSB-R / LSB-M / HILL).
...
changed (|delta|>0), by design. Round-trip is N/A (we never decode the
references). LSB-R/LSB-M are generated in memory (fast); HILL via its Octave simulator
into a folder (then measured and deleted).

Output: results/csv/imperceptibility_reference.csv (config = method, one row per rate).
...
Run (from the repo root):
    python -m scripts.measure.measure_imperceptibility_reference --octave <octave-cli>
"""
import argparse
import csv
import glob
...
from scripts.data.make_reference_sets import _payload, _embed, generate
...
def main():
    ...
    ap.add_argument("--out", default="results/csv/imperceptibility_reference.csv")
