...
Requires Octave once (to read the SCRM submodel layout). Output: results/csv/ml_group.csv.

Run (from the repo root):
    python -m scripts.measure.run_ml_groups --octave <octave-cli>
"""
import argparse
import csv
import os
...
def _baseline_rows(path):
    """Baseline ML-group rows, tagged config=baseline (deterministic 'before')."""
...
def main():
    ...
    ap.add_argument("--out", default="results/csv/ml_group_reanalysis.csv")
    ...
    ap.add_argument("--baseline-from", default="results/csv/ml_group.csv",
                    help="group summary to copy the baseline 'before' rows from")
