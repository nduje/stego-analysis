"""Run SPA over the dataset: detection AND rate estimation (mirror of run_rs.py).

For each embedding rate: embed the covers with the baseline (fixed key, payload
L = round(rate*21760)), get the SPA rate estimate p_hat for every cover and stego
...
Run (from the repo root):
    python -m scripts.measure.run_spa
    python -m scripts.measure.run_spa --limit 20      # quick smoke
"""
import argparse
import base64
import csv
...
from analysis.spa import analyze_image
...
def run(covers_dir, out_scores, out_summary, limit):
    ...
    for idx, path in enumerate(paths):
        ...
        sp = split.get(name, "?")
        cp = analyze_image(load_image(path))
...
def main():
    ...
    ap.add_argument("--scores", default="results/csv/spa_scores.csv")
    ap.add_argument("--summary", default="results/csv/spa_summary.csv")
