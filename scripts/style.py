"""Single source of visual style for the final (print) figures -- Day 21.

Defines the version -> (colour, marker, line) map, the legend order, Croatian display
names for versions/attacks/axes, and two presets (full / half page) applied as
matplotlib rcParams. Drawing code lives elsewhere and is called once per preset, so the
two widths are drawn (not scaled): fonts stay in points at the final figure size.

save_figure writes both a vector SVG (text as paths, so Croatian diacritics survive on
any machine) and a 1200-DPI white-background PNG, into results/figures/final/.
"""
import os
...
def save_figure(fig, name, subdir=""):
    d = os.path.join(FINAL_DIR, subdir) if subdir else FINAL_DIR
    os.makedirs(d, exist_ok=True)
    base = os.path.join(d, name)
    fig.savefig(base + ".svg", bbox_inches="tight", facecolor="white")
    fig.savefig(base + ".png", dpi=1200, bbox_inches="tight", facecolor="white")
    plt.close(fig)
    return base
