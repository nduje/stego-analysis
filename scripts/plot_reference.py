"""Day-19 figures: reference methods (LSB-R / LSB-M / HILL) vs our algorithm.

Reads the *_reference.csv (Day 19) plus our own summaries for context; writes to
results/figures/:
...
Run (from the repo root):
    python -m scripts.plot_reference
"""
import argparse
import csv
import os
...
def ml_fig(out):
    ...
    for m in ("lsbr", "lsbm", "hill"):
        if m in ref:
            ...
            ax.plot(xs, ys, marker="o", color=COLORS[m], label=m)
    # our algorithm for context (from Day-17 ml_reanalysis.csv)
