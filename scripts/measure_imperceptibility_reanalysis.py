...
Same metrics as Day 5 (MSE/PSNR/SSIM, GLOBAL + REGION, per-channel + Y), so the
numbers are directly comparable to the frozen baseline (results/imperceptibility_
summary.csv, E1). Stego is regenerated on the fly (in memory, same key/seed/coverage
as Day 14) and discarded -- no PNGs, no SCRM. The REGION comes from the algorithm's
...
Output: results/imperceptibility_reanalysis.csv (config x rate) with deltas vs baseline.

Run (from the repo root):
    python -m scripts.measure_imperceptibility_reanalysis
    python -m scripts.measure_imperceptibility_reanalysis --limit 20   # smoke
"""
import argparse
import base64
import csv
...
from scripts.make_stego_sets import CONFIGS, KEY
