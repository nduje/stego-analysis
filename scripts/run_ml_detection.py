...
def _baseline_rows(path):
    """Day-10 ml_summary.csv rows, tagged config=baseline (deterministic 'before')."""
...
def run(features_dir, out, n_splits, models, configs, baseline_from):
...
def main():
    ...
    ap.add_argument("--out", default="results/ml_reanalysis.csv")
    ...
    ap.add_argument("--baseline-from", default="results/ml_summary.csv",
