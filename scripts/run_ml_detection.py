...
Output: results/ml_summary.csv (per rate x model).

Run (from the repo root):
    python -m scripts.run_ml_detection
"""
import argparse
import csv
import os
...
def run(features_dir, out, n_splits, models):
    ...
    for rate in EMBEDDING_RATES:
        ...
        assert fc == fs, f"cover/stego filename mismatch at rate {rate}"
        for name in models:
            make = MODELS[name]
            aucs, pes = evaluate_rate(Xc, Xs, make, n_splits)
            rows.append({
                "rate": rate, "model": name, "n_splits": n_splits,
            ...
            print(f"  rate {rate:<5} {name:<9} AUC {aucs.mean():.3f}+/-{aucs.std():.3f}  "
                  f"P_E {pes.mean():.3f}+/-{pes.std():.3f}", flush=True)

    os.makedirs(os.path.dirname(os.path.abspath(out)), exist_ok=True)
    with open(out, "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=["rate", "model", "n_splits",
...
def main():
    ...
    ap.add_argument("--out", default="results/ml_summary.csv")
    ap.add_argument("--splits", type=int, default=N_SPLITS)
    ap.add_argument("--models", default="ensemble,svm", help="comma-sep: ensemble,svm")
    args = ap.parse_args()
    run(args.features, args.out, args.splits, [m.strip() for m in args.models.split(",")])
