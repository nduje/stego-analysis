"""Library demo -- round-trip through the lib pipeline with a passphrase-derived key.

Flow: message -> AES-CTR (key from passphrase: scrypt -> HKDF) -> embed in PNG
-> extract -> AES-CTR^-1 -> message. No sockets.

Run (from the repo root):
    python -m scripts.data.run_stego
    python -m scripts.data.run_stego --cover data/covers/cover_noise.png --message "Hi" --passphrase "correct horse"
"""
import argparse
import os
...
def main():
    ...
    ap.add_argument("--out", default="results/_scratch/stego_lib.png")
