...
Run (from the repo root):
    python -m scripts.data.make_reference_sets --method lsbr --rate 0.05
    python -m scripts.data.make_reference_sets --method lsbm          # all rates
    python -m scripts.data.make_reference_sets --mapping-only          # write the r->bits CSV
"""
import argparse
import csv
import glob
...
def main():
    ...
    ap.add_argument("--mapping-only", action="store_true",
                    help="only (re)write results/csv/reference_payload_mapping.csv")
    args = ap.parse_args()

    write_mapping("results/csv/reference_payload_mapping.csv")
